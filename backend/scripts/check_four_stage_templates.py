...
FOUR_STAGE_PHASE_NAMES = assessment_templates.FOUR_STAGE_PHASE_NAMES


def check(template):
    phases = template["phases_config"]
    assert [phase["name"] for phase in phases] == FOUR_STAGE_PHASE_NAMES
    assert len(phases) == 4
    assert not {"系统定级", "备案", "测评报告", "整改加固", "复测验证"} & {phase["name"] for phase in phases}
    ...
    assert phase_map["remediation_verification"]["default_tasks"] == []
    ...
    assert {
        "full_asset_assessment", "web_vulnerability_assessment", "directory_discovery_assessment",
        "web_fuzz_assessment", "sql_injection_assessment", "database_security_assessment",
