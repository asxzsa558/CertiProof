...
AI_CORE_SYSTEM_PROMPT = """你是 CertiProof 企业等保与商用密码应用安全自查助手。
...
- 流程操作：开始等保/密评自查、整改复测、重置、生成报告，以及材料和流程说明。
- 使用帮助：询问 CertiProof 能做什么或如何操作。
- 询问当前项目能否进行等保或密评，属于使用帮助，不得误判为弱口令检测。
...
DETECTION_INTENTS = {
    ...
    "scan_comprehensive", "scan_crypto", "assessment_technical", "explicit_capability",
}
FLOW_INTENTS = {
...
DETECTION_PLANNER_FALLBACKS = {
    ...
    "scan_crypto": ("crypto_onsite_assessment", {"target": "项目资产", "port": 443}),
}
HELP_RESPONSE = """我可以在当前 CertiProof 项目中帮助你：
- 项目查询：资产、合规状态、通过准备度、主要差距、管理层摘要和已有检测结果。
- 检测执行：端口、漏洞、Web、基线、弱口令、SSL/TLS、数据库等安全检测。
- 流程操作：分别推进等保或密评自查、查看下一步、整改复测、重置当前测评和生成 HTML 报告。
- 使用帮助：说明材料上传、检测参数、结果状态和测评流程。

你可以直接描述目标和动作，例如“对所有资产做 Web 扫描”或“概括当前主要差距”。"""
OUT_OF_SCOPE_RESPONSE = "这个问题不属于 CertiProof 当前支持范围。我只能协助当前项目查询、安全检测、等保或密评自查流程和产品使用操作。"
...
RouteIntent = Literal[
    ...
    "scan_reachability", "scan_comprehensive", "scan_crypto", "assessment_technical", "explicit_capability",
    "assessment_start", "assessment_retest", "assessment_reset", "assessment_status", "assessment_explain",
...
class AIEngine:
    ...
    async def decide(
        ...
        try:
            ...
            if route["category"] == "help":
                return self._immediate_chat(self._help_response(user_input, context), route, capability="help")
    ...
    def _deterministic_plan(self, user_input: str, context: Dict, route: Dict) -> Optional[Dict]:
        ...
        if route["category"] == "project_query" and primary_intent:
            ...
            if capability in {"view_project_status", "view_findings", "view_compliance_score"}:
                parameters["assessment_code"] = (
                    context.get("current_project") or {}
                ).get("active_assessment_code", "dengbao")
        ...
        if route["category"] == "flow_operation" and flow_intent:
            ...
            parameters = {
                "action": action,
                "assessment_code": (context.get("current_project") or {}).get("active_assessment_code", "dengbao"),
            }
            if action == "reset":
        ...
        if route["category"] == "flow_operation" and "report_generate" in route["intents"]:
            ...
            return {
                "plan": [{"capability": "generate_html_report", "parameters": {
                    "project_id": project["id"],
                    "assessment_code": project.get("active_assessment_code", "dengbao"),
                }}],
    ...
    def _route_failure_fallback(self, user_input: str) -> Optional[Dict]:
        ...
        assessment_capability_question = (
            any(marker in text for marker in ("可以", "能否", "能不能", "是否支持"))
            and any(marker in text for marker in ("密评", "密码应用", "等保"))
        )
        assessment_tool_question = (
            any(marker in text for marker in ("工具", "命令", "能力", "怎么检测", "如何检测"))
            and any(marker in text for marker in ("密评", "密码应用", "等保"))
        )
        if (asks_how and (product_topic or "能做什么" in text or "帮助" in text)) or assessment_capability_question or assessment_tool_question:
            ...
            if "密评" in text or "密码应用" in text or any(marker in text for marker in ("国密算法", "国密证书", "tlcp")):
                intent = "scan_crypto"
            elif "端口" in text:
                intent = "scan_ports"
    ...
    def _format_current_project(self, project: Optional[Dict]) -> str:
        ...
        enabled = "、".join(
            f"{item.get('name') or item.get('code')} {item.get('level') or '未定级'}"
            for item in project.get("assessment_types", [])
        ) or "未配置"
        active = "密评" if project.get("active_assessment_code") == "miping" else "等保"
        return f"项目: {project['name']} (ID: {project['id']}, 已启用: {enabled}, 当前视图: {active})"

    def _help_response(self, user_input: str, context: Dict) -> str:
        text = re.sub(r"\s+", "", user_input or "").lower()
        project = context.get("current_project") or {}
        asks_tools = any(marker in text for marker in ("工具", "命令", "能力", "怎么检测", "如何检测"))
        if asks_tools and ("密评" in text or "密码应用" in text):
            return (
                "密评目前分为两类能力：\n"
                "- 自动工具：密码协议与套件核验、数字证书核验、密码应用网络通信组合检测；支持对单资产或项目全部资产执行。\n"
                "- 材料/现场证据检查：补齐八个层面的物理环境、设备计算、应用数据、管理制度、人员管理、建设运行、应急处置等证据。\n\n"
                "自动工具只能形成网络和通信层辅助证据；产品认证、密钥全生命周期和管理控制必须结合上传材料，未覆盖项会标记为“需证据”或“无法判定”，不会自动通过。"
            )
        if asks_tools and "等保" in text:
            return (
                "等保技术检测包括端口、漏洞、Web、配置基线、弱口令、SSL/TLS、数据库、网络设备和 Windows/AD/SMB 检测；"
                "可通过自然语言、快捷指令或 / 命令对单资产和多资产执行。文档合规检查在等保流程的材料上传入口完成。"
            )
        if "密评" not in text and "密码应用" not in text:
            return HELP_RESPONSE
        enabled = {item.get("code"): item for item in project.get("assessment_types", [])}
        if "miping" in enabled:
            level = enabled["miping"].get("level") or "已配置等级"
            return (
                f"可以。当前项目已启用{level}密评自查。你可以切换到“密评”流程，完成密码应用材料检查、"
                "密码应用现场辅助评估、整改复测和 HTML 报告；它与等保共用资产，但进度、问题、评分和报告相互独立。"
            )
        return (
            "可以，但当前项目尚未启用密评。请打开项目设置，编辑项目并勾选“密评自查”、选择二级或三级；"
            "保存后即可在同一项目内切换并推进密评流程。"
        )
