...
- `unable/not_tested/failed/running` 不得伪装成通过，也不直接塞入分母制造假低分；页面必须同时展示可靠检测覆盖率、无法判定数量和不适用数量。仍有未完成正式任务时合规分保持未出具状态。
...
2. 项目测评进度矩阵按项目展示四阶段完成度、问题数、复测状态和下一步动作；进度来自阶段和任务事实，不使用证据数量代替测评进度。
...
顶层工具和组合子工具共用同一结果判定：`scan_completed=false`、`success=false`、`reachable=false` 或 `tool_status=warning` 必须归一化为 `warning`，单独计入“未完成/不可判定”，并展示工具原始原因；不得计入成功、不得生成“0 个问题”或“本次未发现问题”的安全结论。ScanTask 可以进入执行终态 `completed`，但结果可信度必须保持 `conditional`，相关 Finding 和复测项保持活动状态。
...
2. 自然语言默认由 AI Engine 根据当前项目、已授权资产和能力目录选择工具、组合步骤并生成结构化 plan；`/` 命令和快捷按钮只是同一能力目录中的显式工具别名，不建立另一套执行逻辑。
...
14. 自然语言、快捷按钮和 `/` 工具命令只是同一交互检测能力的三种入口，必须创建相同结构的独立 ScanTask；不按入口建立三套队列。Web 扫描 A 与随后提交的端口扫描 B 可以同时运行，谁先完成谁先回写自己的结果，暂停、失败和超时互不改变另一任务状态。
...
| 验收层 | 实际覆盖 | 结果 |
|--------|----------|------|
| 自动化测试 | 文档、图谱、资产范围、队列控制、复测、报告、重置、删除、工具语义、权限边界 | 93 项通过；保留 34 条 Pydantic 2 弃用告警，无测试失败 |
...
| 报告 | HTML 是正式报告格式；第四阶段生成版本化 `ReportArtifact`，保存 JSON 快照、HTML、哈希和过期状态，不保留 PDF 主流程 | 报告中心、文件存储、数据生命周期 | implemented |
...
| 评分口径 | 合规分只基于可靠检查点；检测覆盖率、无法判定和不适用独立展示 | 项目页、完成页、报告 | implemented |
...
| 独立任务调度 | 自然语言、快捷按钮和 `/` 命令共用交互检测队列；不同任务并发执行、独立返回，不因前一任务耗时而阻塞 | 对话、测评、文档、Worker、结果 | implemented |
