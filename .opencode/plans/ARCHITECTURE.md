...
产品采用“组织态势 Dashboard + 项目工作台 + AI 对话执行 + 自动化检测 + 文档合规分析 + 整改复测 + HTML 报告”的闭环设计。用户可以通过项目页自然语言对话或 `/` 命令触发检测，也可以按等保自查流程逐项推进。
...
- 自然语言和 `/` 命令共用同一工具目录与执行契约；项目页不再铺设重复的可点击扫描按钮。
...
- 输入区只保留材料上传、`/` 命令、文本输入和发送，最大限度把垂直空间留给对话与任务结果；所有安全工具继续通过自然语言或 `/` 命令调用。
...
- 左侧等保流程轨道与右侧测评详情面板可分别收起和展开；任一面板的状态不得改变另一面板。
...
- 项目级主导航只展示已经可用的总览、项目、资产、测评、结果、报告、项目设置和组织权限入口，不显示只会弹占位提示的未完成模块。
- 头像菜单中的个人资料展示真实账户、邮箱、角色和当前组织；“设置”进入现有模型与系统设置页，“查看权限”进入组织权限页。项目设置在内容区顶部固定提供醒目的“返回项目对话”，模型设置和组织设置也提供返回入口；返回时优先回到最近一次实际选中的有效项目，失效项目 URL 自动纠正到实际项目，不能让地址栏项目 ID 与页面内容不一致。
...
18. 项目页在测评或阶段运行期间每 5 秒读取持久化状态，刷新或重新进入后继续恢复同一进度。新增检测、重新上传、重置或使旧报告过期属于真实业务写入，可以重新打开阶段并改变进度；单纯浏览页面不能改变进度。
...
入口统一为项目页自然语言、`/` 命令、测评任务和单项复测；所有入口必须解析到同一 Capability Registry 和参数契约。
...
**状态**：入口统一、持久化任务、进度恢复、多任务输入、独立 Worker 队列和受控并发均已 `implemented`；多 Agent 协作是 `accepted-pending`。

1. 用户在 `/projects/:projectId` 输入自然语言或 `/` 命令；回车发送，Shift+Enter 换行，方向上键查看历史输入。输入正文从编辑区左侧正常起笔，材料上传和 `/` 命令位于底部工具带，不能占据正文首列。
2. 自然语言默认由 AI Engine 根据当前项目、已授权资产和能力目录选择工具、组合步骤并生成结构化 plan；`/` 命令是同一能力目录中的显式工具别名，不建立另一套执行逻辑。
...
12. 一个组合计划中某个参数无效或目标越界时，只剔除该子步骤并继续执行其余合法步骤；全部步骤都无效时才拒绝任务。LLM 是自然语言的主规划器；仅当当前指令同时明确写出项目资产和已知工具，而 LLM plan 缺参、引用历史目标或违反工具 schema 时，Orchestrator 才按当前输入做最小纠偏，不扩展成全局固定路由。
...
14. 自然语言和 `/` 工具命令必须创建相同结构的独立 ScanTask；不按入口建立不同队列。Web 扫描 A 与随后提交的端口扫描 B 可以同时运行，谁先完成谁先回写自己的结果，暂停、失败和超时互不改变另一任务状态。
...
19. 删除聊天内泛化的 `view_scan_results` 查询及其自然语言、快捷按钮和命令入口；它过去只返回缓存对象，无法形成可信业务结果。用户查看正式结果应进入项目“检测结果”中心，具体查询继续使用开放端口、漏洞、Finding、评分和扫描历史等明确能力。
...
3. 报告服务读取当前项目、资产、四阶段真实进度、扫描、文档结论、Finding、证据、复测和变更数据，构建一次不可变 JSON 快照；生成报告只能完成报告任务，不能反向把未完成上游阶段强制写成 100%。
...
9. 新安全检测、文档上传/重新分析、整改复测或流程重置会把当前产物标记为 `stale`，记录原因并把报告任务恢复为 `todo`；旧版本保留追溯，重新生成得到下一版本。
...
stateDiagram-v2
  ...
  当前版本 --> 已过期: 新检测/新文档/新复测/重置
  已过期 --> 报告待生成: 报告任务恢复todo
  报告待生成 --> 当前版本: 生成V(n+1)
```

验收要求：测试报告必须同时包含技术初检问题、文档问题、整改动作、技术/文档复测、已修复与仍存在结果；同一版本的 HTML/JSON 哈希和内容保持稳定，新输入必须使旧版本过期。
...
- 正式报告由第四阶段生成并保存为递增版本；预览、HTML 和 JSON 使用同一快照，新检测或材料使旧版本明确过期。
...
**执行日期**：2026-07-18。下列结果来自当前 Docker Compose 运行环境，不是手工修改数据库得到的演示数字。

| 验收层 | 实际覆盖 | 结果 |
|--------|----------|------|
| 自动化测试 | 文档、图谱、资产范围、队列控制、复测、报告、重置、删除、工具语义、权限边界、评分回填、测评幂等初始化 | 101 项通过；仅保留 Pydantic 2 弃用告警，无测试失败 |
...
| 进度一致性 | 单项目唯一测评、确定性最新记录、只读整改工作区、运行态轮询和刷新恢复 | 迁移 `020` 已删除空重复实例；项目 72 在轮询 11 秒和刷新后均稳定为 75%，数据库无重复测评实例 |
| UI | Dashboard、项目与资产、报告中心、项目执行页、检测列表、模型设置、组织权限；桌面 1280px | 无页面级横向溢出、无控制台错误；组织权限异步加载后内容完整 |
...
| 项目工作台 UI | 项目执行页采用约 88px 图标文字导航、约 332px 四阶段流程轨道、弹性中央 AI 对话和宽屏约 560px 测评详情；输入区只保留上传、`/` 命令和发送，任务与结果卡可点击展开，顶部指标固定对齐；左右业务面板可独立收起；问题状态打开独立筛选详情；页头徽标 3D 水平翻转，对话头像和水印静止 | `/projects/:projectId` | implemented |
| 项目资产与设置 | 当前项目资产在项目设置中维护并明确显示资产类型和所属项目；跨项目资产留在统一资产视图；项目、模型和组织设置均可返回最近有效项目对话，失效 URL 自动纠正 | 项目设置、模型设置、组织设置、资产 | implemented |
| 技术检测 | `/` 命令和自然语言共用能力目录，多资产结果统一聚合；泛化的缓存“查看扫描结果”能力已删除，正式记录从检测结果中心查看 | 对话、任务、结果 | implemented |
| 报告 | HTML 是正式报告格式；第四阶段生成版本化 `ReportArtifact`，保存 JSON 快照、HTML、哈希、评分快照和过期状态；项目页和报告中心均从同一历史接口读取版本 | 报告中心、项目页、文件存储、数据生命周期 | implemented |
...
| AI 编排与契约兜底 | 自然语言由 LLM 选择和组合工具；显式工具别名、当前资产、权限与参数 schema 由 Orchestrator 做最小纠偏，Web 扫描别名使用 Nikto 的 `target` 契约 | AI Engine、Orchestrator、资产范围 | implemented |
| 独立任务调度 | 自然语言和 `/` 命令共用交互检测队列；不同任务并发执行、独立返回，不因前一任务耗时而阻塞 | 对话、测评、文档、Worker、结果 | implemented |
| 查询结果契约 | 资产清单等管理查询使用确定性摘要和 `query_result`，资产总数、名称、类型、地址、验证状态可直接追溯，不由模型改写为扫描结论 | AI 对话、Orchestrator、结果卡 | implemented |
