...
| `/settings/system` | 系统设置 | 查看硬件建议并配置部署级资源档位、并发、背压、推理策略和文档分析默认模式 |
| `/settings/models` | 模型与推理端点 | 配置 AI 模型、供应商、用途和连接状态 |
...
- `acceptance` Compose profile 提供按需启动的单容器多协议靶机 `e2e-target`，受控开放 SSH、HTTP、HTTPS、SNMP、Oracle、MySQL、Redis、Memcached 和 MongoDB，并提供固定 Web 路径、弱口令和不安全配置。`scripts/acceptance-lab.sh` 统一执行启动、重置、停止、状态、能力清单以及快速/完整矩阵；靶场不属于生产服务，也不替代已授权公网抽样。
- 验收脚本通过现有 MCP Gateway 和 Tool Server 调用真实工具，不在后端挂载 Docker Socket。项目包含 `e2e-target` 资产时，项目页只显示紧凑的验收环境状态，并复用既有对话任务卡展示每个工具的实时进度、资产归属和结果详情，避免另建一套任务系统。
...
受控靶场是按需启动的测试环境，不随 CertiProof 生产服务常驻。**项目页不能启动 Docker 靶场，首次使用或停止后再次使用前，必须先在项目根目录执行启动脚本。** 页面中的“受控工具验收环境”只说明当前项目包含验收资产，不代表容器一定正在运行。

1. 启动靶场：执行 `./scripts/acceptance-lab.sh start`。脚本构建并启动 `e2e-target`，等待所有受控协议健康后返回。
2. 检查状态：执行 `./scripts/acceptance-lab.sh status`。只有状态健康时才进入项目页测试。
3. 准备项目：在项目资产中添加且只添加一次主机名 `e2e-target`；同一目标的重复资产必须由资产层去重。
4. 执行工具：进入具体项目页，通过自然语言对话、可视化快捷指令或 `/` 快捷命令发起端口、Web、TLS、弱口令、基线、数据库或 SNMP 等检测。三种入口共用同一执行契约和独立任务队列。
5. 查看过程：项目页立即创建任务卡，展示运行状态、当前资产、工具、结果摘要、发现详情和具体错误；刷新后从后端任务记录恢复，不依赖页面驻留。
6. 重复验证：执行 `./scripts/acceptance-lab.sh reset` 重建干净靶场，然后从项目页重新发起检测。该操作不删除项目资产、聊天或历史结果。
7. 停止环境：不再测试时执行 `./scripts/acceptance-lab.sh stop`，释放靶场容器占用的资源。
8. 自动矩阵：`./scripts/acceptance-lab.sh matrix quick` 执行常用快速能力，`./scripts/acceptance-lab.sh matrix full` 增加 Nikto、Nuclei 和 SQLMap；结果分别写入 `artifacts/security-tools-quick-acceptance.json` 和 `artifacts/security-tools-full-acceptance.json`。
9. 查看清单：执行 `./scripts/acceptance-lab.sh list`，区分本靶场已覆盖能力与仍需 Windows VM、交换机、密码机或其他授权实机的能力。

```mermaid
flowchart LR
  SCRIPT["运维脚本启动 e2e-target"] --> HEALTH["status 确认协议健康"]
  HEALTH --> ASSET["项目资产包含 e2e-target"]
  ASSET --> ENTRY["对话 / 快捷指令 / 斜杠命令"]
  ENTRY --> TASK["独立任务队列与 MCP 工具执行"]
  TASK --> CARD["项目页任务卡：进度、结果、错误"]
  RESET["reset 重建干净靶场"] --> HEALTH
  STOP["stop 释放资源"] -.-> SCRIPT
```
...
组织
  ...
  └── 组织设置
      ...
      ├── 扫描节点：远端执行面配置和状态
      ├── 系统设置：部署级资源、并发、背压和文档默认策略
      └── 模型与推理端点：LLM 供应商、用途和连接测试
```
...
产品目标：
- 作为用户进入系统后的指挥台。
- 展示“项目整体进度、精确问题追踪、资产暴露面、工具健康度、运行与告警摘要”。
- 不承载对话指令，避免与项目执行页重复。
...
- 顶部只保留项目数和资产数两个可直接核验的组织指标，并分别拆出进行中/已完成项目以及 IP/域名/云资源构成；不再展示来源不清的聚合风险数字。
- 问题追踪：按待处理/已处置筛选当前 Finding；每行必须展示所属项目、实际资产或“项目级问题”、问题名称、来源工具/文档、严重度、状态、出现次数和最近时间，点击后进入对应项目的等保、密评或独立检测工作区。
...
- 项目工作区固定为 `等保 / 密评 / 独立检测` 三类。等保和密评显示各自四阶段、评分与报告；独立检测集中展示自然语言、快捷指令、`/` 命令和定时检测产生的可追踪问题，并明确提示这些问题尚未计入正式测评分。
...
密评标准图谱当前覆盖 13 类材料、56 个检查项、112 个必检证据点，工程口径依据 GB/T 39786-2021、GB/T 43206-2023、GB/T 43207-2023、GM/T 0116-2021、《商用密码应用安全性评估高风险判定指引》《商用密码应用安全性评估测评工具指引》和《商用密码应用安全性评估管理办法》组织。每类材料和检查项均记录适用范围、自动化边界、依据、严重程度、必需证据、完整性条件、否定条件和整改建议；高风险口径显式覆盖违规算法协议、密钥全生命周期、密码产品合规性及关键传输保护。标准库、文件内容块、向量召回、图谱关系、模型判证和规则裁决沿用 4.7 的统一文档链路，但使用独立的密评分库与 `assessment_id`。缺少材料、工具条件、现场证据或模型可靠结论时必须为 `pending/unable/not_tested`，不得自动通过。该库已经完成基于公开权威标准的工程校准，但产品输出仍是企业内部自查结论，正式密评结论和项目特定适用性最终确认仍由具备资质的评估机构负责。
...
- Nmap、Masscan、Nuclei、Hydra、TestSSL、密码传输检测、Nikto、SQLMap、Gobuster、FFUF、SNMP、Windows/SMB 和 SSH 基线原子工具统一使用异步长任务契约，不设置默认总运行时长；实际进程仍存活且持续产生心跳时继续执行。工具服务每 2 秒更新运行秒数、存活状态和估算进度，Worker 将最新进度持久化到 `ScanTask.progress` 并续租，WebSocket 和刷新后的轮询读取同一份状态。
- Gateway 同步 `/call` 只承载 Ping、Fping 和五类数据库连接等有界协议探测；长工具误用同步入口时返回 `409`，官方客户端自动升级到 `/call/async`，从而保留进度、心跳、刷新恢复和取消。短探测保留连接/协议超时，因为“在规定握手窗口内无响应”本身就是检测事实，不等同于长扫描的总墙钟时限。
- 单次进度请求的网络超时不等于工具停止。Gateway 轮询短暂中断时显示“正在重连”并继续；只有连续 90 秒无法获得工具心跳时，才以“执行状态无法确认”结束本次调用。用户主动停止仍立即贯通 Worker、Gateway 和工具进程。
...
- 自然语言、快捷指令、`/` 命令和定时检测发现可靠风险时，按“项目 + 资产 + 原子能力 + 风险指纹”创建或更新独立 Finding；同一问题重复出现只增加出现次数和最近时间，不生成重复卡片。
- 执行失败、检测不完整、目标不可达和 `not_tested` 只保留为检测执行记录，不创建安全风险 Finding。只有同一资产、同一原子能力后续得到完整可信的无风险结果，才自动关闭对应独立 Finding。
- 独立 Finding 不改变等保/密评阶段、合规分和已生成报告。用户从独立检测工作区明确选择“纳入等保”或“纳入密评”后，系统创建带 `origin_finding_id` 的正式 Finding，后续进入该测评的整改与复测闭环。
...
8. WebSocket 推送实时状态，轮询作为补充；完成事件必须携带完整结果。Nmap、Masscan、安全、Web、SNMP、Windows/SMB 和 SSH 基线长工具共用 `start/progress/result/cancel` 接口，按心跳持续写入存活状态、运行秒数和估算进度，不以固定墙钟时长强制停止；页面刷新后根据持久化任务恢复运行态或已完成结果。
9. 前端把同一命令的多资产结果聚合为一张可展开卡片；每个资产显示 IP/域名、资产名称、工具、覆盖内容、风险和错误。
10. TaskExecutor 只从完整成功结果提取风险。正式测评任务生成测评 Finding；交互和定时任务生成独立 Finding。失败、警告、部分执行或不可验证不能生成“无风险”结论，也不能关闭已有问题。
11. 自动测评任务每 10 秒续租，租约默认 2 分钟；每个资产完成后立即写入 `asset_results` 检查点，Worker 恢复时跳过已完成资产，最多尝试 3 次。
12. 停止请求先以 `cancel_requested_at` 和 `execution.state=cancelled` 持久化，再由 Worker 取消本地协程；MCP Client 调用 Gateway 取消接口，Tool Server 终止对应 Nmap、Masscan、安全、Web、SNMP、Windows/SMB 进程或关闭 SSH 协程。
...
16. `interactive` 和 `scheduled_monitoring` 结果保留在检测记录、独立 Finding、风险信息和端口变化提示中，但不重算当前测评分、不直接创建测评技术 Finding，也不使已生成报告过期。
17. 独立问题使用资产、原子能力和稳定风险指纹幂等更新；重复发现增加 `occurrence_count` 和 `last_seen_at`，组合扫描中的每个真实子工具保留自己的来源能力。
18. 只有同一资产、同一原子能力的完整可信复扫明确没有对应风险时，独立问题才自动转为 `fixed`；超时、不可达、认证失败和检测不完整均保持原问题状态。
19. 用户可从独立检测工作区重新检测问题，或明确纳入当前项目已启用的等保/密评。纳入操作复制为正式 Finding 并记录 `origin_finding_id` 和事件，不修改原始独立问题的审计事实。
20. 独立检测工作区与正式测评共用中央对话，但左侧显示独立问题摘要，右侧按全部、待处理、已修复筛选问题卡；Dashboard 深链必须能定位到对应工作区和问题。
...
sequenceDiagram
  ...
  alt 全部或部分子任务已结束
    ...
    W->>DB: 保存执行状态无法确认及底层连接原因
    DB-->>FE: 显示检测不完整，不生成无风险结论
  else 用户停止
    FE->>API: 停止 task_id
...
端口结果必须区分 `open`、`closed`、`filtered`、`unresolved`、`unreachable`；SSH 端口扫描显示开放只代表 TCP 可建立，基线检查仍可能出现认证失败、连接拒绝或容器出口超时。SSL/Web/数据库等协议工具同样必须区分端口层可达和应用层握手成功。

Web 类工具执行前共用协议预检：解析域名并探测显式端口，未指定协议时依次探测 HTTPS 443 和 HTTP 80，选择第一个明确开放的服务；再请求入口页和随机不存在路径，记录有效 URL、HTTP 状态、软 404、响应长度以及 WAF/验证码标识。Nikto、Gobuster 和 FFUF 遇到访问挑战时提前停止并返回覆盖受限，避免把统一挑战页当漏洞或目录；Nuclei 默认使用 `safe` 档位，普通目标限制为 `cve,misconfig,exposure + critical/high/medium`，受保护目标缩小为 `misconfig,exposure + critical/high`，并降低速率与并发。用户明确选择 `full` 时才进入完整模板范围。通用 Nuclei 即使没有 80/443，也继续使用自身多服务端口预检，不能漏掉 SSH 等非 Web 服务。

SSL/TLS 在 443 明确关闭时返回“不适用”；端口超时、被过滤、无法解析或握手失败时返回“检测不完整”；只有端口开放并完成 testssl 协议分析后，才可以形成证书、协议和漏洞结论。

错误结果必须包含：发生在哪个资产、哪个工具、执行到哪一步、底层原因、是否可以重试以及建议检查的网络、凭据或参数。复制结果时递归序列化对象和数组，不能出现 `[object Object]`。

顶层工具和组合子工具共用同一结果判定：`scan_completed=false`、`success=false`、`reachable=false` 或 `tool_status=warning` 必须归一化为 `warning`，界面显示“检测不完整”并展示工具原始原因；明确存在安全发现显示“发现问题”；二进制、路由或参数失败显示“执行失败”；目标明确未提供对应服务显示“不适用”；其余完整且无发现才显示“检测完成”。检测不完整不得计入成功，不得生成“0 个问题”或“本次未发现问题”的安全结论。不适用单独计数并从评分分母排除，不能混入失败或未完成。状态和风险标签禁止拆字换行，长原因允许自然换行或横向滚动，不能压缩成不可读文本。ScanTask 可以进入执行终态 `completed`，但结果可信度必须保持 `conditional`，相关 Finding 和复测项保持活动状态。
...
Finding 通过 `source_channel` 区分 `assessment`、`interactive` 和 `scheduled_monitoring`，通过 `asset_id` 绑定实际资产，通过 `last_seen_at` 与 `occurrence_count` 表达重复发现；被纳入正式测评的问题使用 `origin_finding_id` 追溯独立发现。独立 Finding 是正式测评之外的持续问题台账，不参与测评分；正式 Finding 才进入以下整改与复测流程。
...
5. `/settings/system` 是部署级控制面，明确标注“全局生效”；展示 CPU、可用内存、GPU、当前压力、推荐档位和有效并发。自动模式采用推荐值，手动模式支持轻量、标准、GPU 和自定义档位。
6. 自定义档位统一控制交互扫描、技术测评、文档分析、整改复测和模型调用并发。所有任务生产者和 Worker 都读取同一个运行时有效额度，不允许绕过到静态环境变量。达到资源压力阈值时暂停领取新任务，压力恢复后自动继续。
7. 部署控制的批量保存先完整校验再在同一事务写入；未知键、空修改和越界值明确拒绝，并记录操作者、组织、变更键和前后值到追加式审计事件。配置是部署全局状态，只有当前组织具备 `system:config` 的成员可修改。
8. 单文件/批量上传上限、文档总页数、失败重试、活动对话保留期和任务执行模式属于启动级边界：系统设置只读展示，管理员通过环境变量修改并重启对应服务。前端不提供没有运行时消费者的假开关。
9. 文档解析模式包括原生解析优先、轻量 OCR fallback、完整文档视觉模型三层。
10. 标准模式在原生结果为空或可疑时自动补充 OCR；深度模式对复杂页面做视觉交叉验证。
11. OCR 默认在本地容器处理企业材料；部署到不同 CPU/GPU 环境时允许切换推理后端，但 OCR Service 和 MCP Gateway 对外保持统一可路由的 `document_page_parse` 契约。
12. 模型不可用、超时或崩溃必须被隔离为单页/单任务错误，不能导致 Worker 永久占用或把检查判为通过。
13. 模型密钥和文档凭据不得出现在 API 返回、日志、报告和架构文档中。
...
| 系统设置 | 部署级硬件建议、推理策略、动态并发、背压阈值、文档默认模式与只读启动边界 | `/settings/system` | implemented |
...
| 问题 | 当前边界 | 已确认处理方向 |
|------|----------|----------------|
| 标准库外部专业复核 | 等保和密评标准库已完成公开权威标准的工程校准，但不冒充持证测评人员或有资质机构结论 | 正式商用前由业务专家复核项目适用性、地方要求和标准更新，复核结果通过版本化标准库发布 |
...
| 验收层 | 实际覆盖 | 结果 |
|--------|----------|------|
| 自动化测试 | 文档、图谱、资产范围、队列控制、复测、报告、重置、删除、工具语义、权限边界、评分回填、等保/密评双实例隔离、密评八层面矩阵、国密工具失败语义、五类对话契约、确定性项目查询、检测变化、上下文续接、受控整改建议、Prompt Skill 路由、能力白名单、拓扑风险口径、模型结构化输出、运行时策略、资源档位、配置原子保存、健康探针、认证保护、远端节点路由边界、Web 预检策略、异步工具契约和验收靶场契约 | 246 项通过，无代码测试失败；前端 lint 与生产构建通过 |
...
| 标准与检索 | 等保 10 类核心文档、40 个控制域、80 个必检项；密评 13 类材料、56 个检查项、112 个必检证据点；两个版本化标准图谱独立入库，使用精确检索 + 1024 维向量 + AGE 归属 | 通过；密评库已补全适用性、自动化边界、高风险条件和权威依据 |
...
| 密评真实闭环 | 保留项目 `#102` 使用 13 份初检 DOCX、13 份整改 DOCX、八层面矩阵、真实 TLS/证书辅助检测、材料复核、技术复测和 HTML 报告 | 初检 100 个问题；整改后 45 个已修复、62 个待处理、3 个无法验证；当前报告 V3 为 `artifacts/certiproof-real-miping-report.html`，明确区分流程 100% 与“未通过自查”结论 |
| 交互并发 | 连续提交 Web 扫描与双资产端口扫描，重复资产去重，独立任务 ID 和结果归属 | 当前镜像两次创建耗时 0.058 秒和 0.017 秒；结果分别归属 `e2e-target`、`redis` |
| 工具层受控矩阵 | `./scripts/acceptance-lab.sh matrix full` 经 MCP Gateway 真实执行 Ping、Fping、Nmap、Masscan、TestSSL、密码传输与证书、Gobuster、FFUF、Linux 基线、Hydra、五类数据库、SNMP Walk/Get/团体字、Nikto、Nuclei 和 SQLMap | 21/21 通过；另有 Nuclei 实际进程停止控制通过。Nikto 返回 11 项发现、Nuclei 连续运行 139.28 秒并返回 4 项发现，两个目录工具各返回 6 条，弱口令与数据库暴露均被实际识别；完整矩阵 JSON 保存于 `artifacts/security-tools-full-acceptance.json` |
| 工具层外部边界 | Windows/AD/SMB 与交换机、密码机、国密硬件 | 前者需要 Windows VM，后者需要授权实机；能力清单明确标注，不能用 Linux 轻量靶场冒充验证通过 |
| 已授权公网抽样 | `121.40.95.31` 高危端口、SSL、Web | 主机可达，23 个端口为 filtered；应用层未完成时保留具体原因，未误报“无风险”。当前长工具不再使用历史固定总时限，后续公网结果以进程心跳和目标协议事实为准 |
| 域名与 WAF 抽样 | `wisetone.com.cn` 协议预检、Nikto、Gobuster、FFUF、Nuclei、SSL | 自动选择 HTTP 80 并识别长亭雷池挑战；三个 Web 发现工具约 2.7 秒提前停止；Nuclei 64.85 秒以安全档位结束并保持覆盖受限；443 filtered 在 2.5 秒内返回检测不完整 |
| 本地 TLS 真实工具 | `e2e-target:443` 自签名 HTTPS | testssl 98.36 秒完成，识别 TLS 1.2/1.3、自签名与域名不匹配、6 个配置问题和 1 个严重评级 |
...
验收边界：公网目标受容器出口、防火墙和限速影响，工具“超时/filtered”是可信执行事实而不是平台故障；当前容器的完整文档视觉模型依赖 fallback 保证扫描件可用，复杂图表理解仍需在目标生产硬件上复验；标准库已完成公开权威标准工程校准，正式密评结论和项目特定适用性仍需有资质机构确认；多 Agent 与离线压缩导出仍为后续能力。本轮按验收计划未执行云服务器实机验收。
...
| 密评八层面 | 密评使用 13 类材料、56 个检查项、112 个必检证据点；八层面矩阵由持久化任务、文档结论和技术 Finding 统一计算，页面与 HTML 报告共用同一后端结果；自动工具只覆盖网络通信辅助核验，正式结论不替代有资质机构 | 密评、文档、工具、整改、报告 | implemented |
...
| 独立问题追踪 | 交互与定时检测的可靠风险按项目、实际资产、原子能力和指纹幂等追踪；失败/不完整只记执行事实，可信同能力复扫才关闭；用户明确纳入后才复制到等保/密评并记录来源关系 | 项目独立检测工作区、Dashboard、Finding、扫描任务 | implemented |
| Web 预检与扫描档位 | Web 工具执行前统一完成协议选择、端口、随机路径、软 404 和 WAF/验证码预检；Nikto/目录工具遇阻断提前结束，Nuclei 默认安全档位并按防护降压，完整档位需用户明确选择 | Execution Engine、Nuclei、Web 工具、结果卡 | implemented |
...
| 工具服务边界 | 安全工具集中在 `security-tools`，高速扫描、Web、网络、Windows、数据库和 SSH 各保留一个权威 Tool Server；已删除未被 Compose/Gateway 使用且仍保留旧超时语义的 Nmap、Nuclei、Hydra、TestSSL 重复服务 | MCP、Compose、部署 | implemented |
| 长任务可靠性 | 文档按文件、技术检测按资产保存检查点；Nmap、Masscan、安全、Web、SNMP、Windows/SMB 和 SSH 基线长工具共用异步生命周期，进程存活即持续心跳、持久化运行秒数和估算进度；同步绕行被 Gateway 阻止并由客户端自动升级；短暂轮询断线自动重连，连续 90 秒无心跳才判定状态无法确认；复测长工具调用由独立监视器持续续租；停止贯通 Worker、Gateway 与 Tool Server | 文档、测评任务、复测、MCP、UI | implemented |
...
| 受控验收环境 | Compose `acceptance` profile 内置按需启动的多协议 `e2e-target`；统一脚本支持启动、重置、停止、状态、能力清单和 quick/full 矩阵，项目页复用现有任务卡显示过程与结果；Windows/AD 和实机类能力保持外部验收边界 | 部署、工具、项目执行页、CI/本地验收 | implemented |
...
| 资源自适应 | `/settings/system` 展示部署级 CPU/内存/GPU 建议，支持自动/手动档位、五类并发和背压阈值；所有任务生产者与 Worker 使用同一有效额度，批量保存原子校验并审计 | 系统设置、Worker、模型调用、Compose | implemented |
