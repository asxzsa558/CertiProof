...
def test_scan_record_deletion_removes_all_dependents():
    async def run():
        ...
        async with session_factory() as db:
            ...
            await db.flush()
            finding = Finding(project_id=project.id, scan_task_id=task.id, clause_id="TECH-TEST", clause_name="测试检测", severity=Severity.HIGH, judgment=Judgment.FAIL, judgment_engine=JudgmentEngine.RULE, status=FindingStatus.OPEN)
            db.add(finding)
            await db.flush()
            db.add_all([
                RemediationTicket(finding_id=finding.id, project_id=project.id, title="test", status=RemediationStatus.OPEN),
            ...
            await db.commit()
            for model in (ScanTask, Finding, RemediationTicket, Evidence, ScanHistory, ChangeSnapshot):
